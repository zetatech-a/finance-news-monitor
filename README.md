...
- 검증을 통과한 AI 3줄은 **마크다운 리포트에서도 자르지 않습니다.** 길이가 이미 계약
  (3줄 × 줄당 `GEMINI_MAX_LINE_CHARS`)으로 묶여 있어서, 여기에 고정 한도를 또 걸면
  한도를 기본(90)보다 올렸을 때 마크다운에서만 세 번째 문장이 사라져 HTML 리포트와
  내용이 어긋납니다. 길이가 보장되지 않는 추출요약·스니펫은 기존대로 잘라 씁니다.
