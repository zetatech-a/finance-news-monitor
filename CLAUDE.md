...
|------|---------|--------------|
| `daily.yml` | Cron **every day**: 23:41/23:49/23:57 UTC + 00:07/00:17 UTC (= KST 08:41/08:49/08:57/09:07/09:17); `workflow_dispatch` | phase5 sent-marker precheck → optional wait until ~08:55 KST → candidate model refresh → `run_daily --end_hhmm 0855` → email (if secrets + no marker) → write marker → commit `reports/` to `main` |
